...
export async function listMcpServers() {
  const response = await http.get<ApiResponse<McpServer[]>>("/api/mcp/servers");
...
export async function listMcpTools(serverId?: number) {
  const response = await http.get<ApiResponse<McpTool[]>>("/api/mcp/tools", {
    params: serverId ? { serverId } : undefined
  });
  return response.data.data;
}

export async function listAgentToolBindings(agentId: number) {
  const response = await http.get<ApiResponse<AgentToolBinding[]>>(`/api/agents/${agentId}/tool-bindings`);
  return response.data.data;
}
